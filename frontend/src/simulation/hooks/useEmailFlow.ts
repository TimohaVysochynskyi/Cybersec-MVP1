...
export const useEmailFlow = ({ allEmails }: UseEmailFlowProps) => {
    ...
    const addNewEmail = useCallback((email: EmailType) => {
        const updatedEmail = {
            ...email,
    ...
    // Початкове додавання першого листа через 3 секунди
    useEffect(() => {
        if (allEmails.length > 0 && receivedEmails.length === 0) {
            const timer = setTimeout(() => {
                addNewEmail(allEmails[0]);
            }, 3000);

            return () => clearTimeout(timer);
        }
    }, [allEmails, receivedEmails.length, addNewEmail]);
    ...
    const startTraining = useCallback(() => {
        if (!isTrainingStarted && allEmails.length > 1) {
            ...
            setTimeout(() => {
                addNewEmail(allEmails[1]);
            }, 2000); // Збільшую затримку для кращого UX
        }
    }, [isTrainingStarted, allEmails, addNewEmail]);
    ...
    const proceedToNextEmail = useCallback(() => {
        ...
        if (nextIndex < allEmails.length) {
            ...
            setTimeout(() => {
                addNewEmail(allEmails[nextIndex]);
            }, 3000);
        }
    }, [currentEmailIndex, allEmails, addNewEmail]);
